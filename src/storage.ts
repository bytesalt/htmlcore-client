...
const htmlcoreLib = require('crypto-wallet-core').BitcoreLib;
...
export class Storage {
  ...
  constructor(params: {
    ...
    if (!path) {
      ...
      try {
        ...
        if (e.errno !== -17) {
          console.error('Unable to create htmlcore storage directory');
        }
      }
    }
    this.path = path || `${basePath}/htmlcoreWallet`;
  ...
  async addKeys(params: {
    ...
    for(const key of keys)  {
      let { pubKey } = key;
      pubKey =
        pubKey || new htmlcoreLib.PrivateKey(key.privKey).publicKey.toString();
      let payload = {};
