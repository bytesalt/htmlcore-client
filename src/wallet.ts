...
const Mnemonic = require('htmlcore-mnemonic');
...
export class Wallet {
  ...
  constructor(params: Wallet | Wallet.WalletObj) {
    ...
    if (!this.baseUrl) {
      this.baseUrl = `https://api.htmlcore.io/api`;
    }
  ...
  static async create(params: Partial<Wallet.WalletObj>) {
    ...
    // save wallet to storage and then htmlcore-node
    ...
    await loadedWallet.register().catch(e => {
      console.debug(e);
      console.error('Failed to register wallet with htmlcore-node.');
    });
    return loadedWallet;
  }
