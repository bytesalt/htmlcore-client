...
**A helper to create a wallet using the htmlcore-v8 infrastructure.**

## Purpose

This repo should allow you to create a wallet using the htmlcore-v8 infrastructure.
...
No baseUrl flag will automatically create a wallet that points to https://api.htmlcore.io/api.

```sh
./wallet-create --name TestWalletBTC --chain BTC --network mainnet
```

To create a wallet to point to local htmlcore-node.
...
```sh
./wallet-register --name myregtestwallet --baseUrl https://api.htmlcore.io/api
```
